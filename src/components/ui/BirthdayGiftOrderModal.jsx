...
export default function BirthdayGiftOrderModal({ isOpen, onClose, selectedGift }) {
  ...
  const basePrice = Number(selectedGift.price) || 499;

  // Finish extras
  const finishExtra = finishType === "gloss" ? 150 : finishType === "deluxe" ? 299 : 0;
  const finishLabel = finishType === "gloss" 
    ? "Ultra-Gloss UV Protective Layer" 
    : finishType === "deluxe" 
    ? "Deluxe Combo (Gift Box + Chocolates)" 
  ...
  const handleDispatchWhatsApp = () => {
    ...
    const receipt = 
      `🎁 *NEW PERSONALIZED BIRTHDAY GIFT ORDER* 🎁\n` +
      `----------------------------------------\n` +
      `🎀 *Item:* ${selectedGift.name}\n` +
      `✨ *Finish:* ${finishLabel}\n` +
      `🔢 *Quantity:* ${quantity} Units\n` +
      `💵 *Unit Price:* ₹${(basePrice + finishExtra).toLocaleString("en-IN")}\n` +
      ...
      `----------------------------------------\n` +
      (isGift ? `🎀 *GIFT WRAPPING & WISH:*\n` +
        (recipientName.trim() ? `• *For Recipient:* ${recipientName.trim()}\n` : "") +
        `• *Wish Message:* "${giftWish.trim() || "Happy Birthday / Best Wishes!"}"\n` +
        `• *Packaging:* Free Luxury Gift Wrapping & Handwritten Card\n` +
        `----------------------------------------\n` : "") +
      `👤 *CLIENT DETAILS:*\n` +
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
        ...
        >
          ...
          <div className="p-5 sm:p-6 border-b border-white/10 flex items-center justify-between bg-black/30">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs uppercase font-extrabold tracking-wider text-teal-400 flex items-center gap-1">
                  <Gift size={13} /> Personalized Birthday Gift Studio
                </span>
          ...
          <div className="p-5 sm:p-6 space-y-5">
            ...
            {/* STEP 1: Select/Upload Custom Photo */}
            {step === 1 && (
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-[#081210] border border-white/10">
                  <img
                    src={selectedGift.image}
                    alt={selectedGift.name}
                    className="w-16 h-16 rounded-xl object-cover border border-teal-500/30 shrink-0"
                  />
                  <div>
                    <h4 className="text-sm font-bold text-white">{selectedGift.name}</h4>
                    <p className="text-xs text-teal-300 font-serif font-bold mt-0.5">Base Price: ₹{basePrice}</p>
                    <p className="text-[11px] text-zinc-400 font-light">Customized with your personal photo</p>
                  </div>
                </div>

                <p className="text-xs text-zinc-300 font-light">
                  Upload your memorable birthday or anniversary photo to print and customize onto this gift.
                </p>
                ...
                <div className="pt-2 flex justify-end">
                  <button
                    onClick={() => {
                      if (!photoPreview) {
                        setErrorMsg("Please upload a photo for the gift to proceed.");
                        return;
                      }
                      setErrorMsg("");
                      setStep(2);
            ...
            {/* STEP 2: Customization & Details (MATCHES USER SCREENSHOT EXACTLY!) */}
            {step === 2 && (
              <div className="space-y-4">
                {/* Finish Options */}
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-300 mb-2">
                    SELECT FINISH &amp; PACKAGING
                  </label>
                  <div className="grid grid-cols-3 gap-2.5">
                    {[
                      { id: "standard", title: "Standard Finish", badge: "CLASSIC", extra: "+₹0" },
                      { id: "gloss", title: "UV Gloss Layer", badge: "ULTRA-GLOSS", extra: "+₹150" },
                      { id: "deluxe", title: "Gift Box Combo", badge: "DELUXE", extra: "+₹299" },
                    ].map((fin) => (
                      <button
                        key={fin.id}
                        type="button"
                ...
                {/* Quantity */}
                <div className="flex items-center justify-between p-3.5 rounded-2xl bg-[#081210] border border-white/10">
                  <div>
                    <span className="text-xs font-semibold text-white block">Order Quantity</span>
                    <span className="text-[11px] text-zinc-400">Total units of this gift</span>
                  </div>
              ...
              <div className="space-y-5">
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-r from-[#0d2a22] to-[#091b16] border border-teal-400/40 shadow-xl flex items-center justify-between">
                  ...
                  <div className="text-right text-xs text-zinc-300">
                    <span className="block font-bold">{selectedGift.name}</span>
                ...
                >
                  ...
                  <span>Submit Gift Order to SSS Studio WhatsApp</span>
                </button>
