...
export default async function AdminPackages() {
 ...
 <Plus className="text-brand-gradient" /> Add New Package
 </h2>
 ...
 <button 
 type="submit" 
 className="w-full bg-brand-gradient hover-glow-brand text-black font-bold py-3 px-4 rounded-xl hover:shadow-lg hover:shadow-cyan-500/30 transition-all mt-4"
 >
 Save Package
 </button>
 </form>
 ...
 {pkg.popular && (
 <div className="absolute top-0 right-0 bg-brand-gradient hover-glow-brand text-black text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-bl-lg rounded-tr-2xl">
 Popular
 </div>
 )}
 <h3 className="text-xl font-bold text-white mb-1">{pkg.name}</h3>
 <div className="text-2xl font-bold text-brand-gradient mb-4">{pkg.price}</div>
 ...
 <Check className="w-3.5 h-3.5 text-brand-gradient shrink-0 mt-0.5" />
 <span>{feature}</span>
 </li>
 ))}
 </ul>
