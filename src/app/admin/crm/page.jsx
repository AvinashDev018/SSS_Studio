...
import { Copy, Plus, ArrowRight, MessageCircle, Check, Trash2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-zinc-950 p-6 sm:p-12 text-zinc-100 font-sans">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 flex gap-3 text-amber-200 text-sm">
          ...
          <div>
            <p className="font-bold text-amber-500 mb-1">How this CRM works (No Database)</p>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl">
          <h2 className="text-xl font-bold text-white flex items-center gap-2 mb-6">
            <MessageCircle className="text-amber-500" /> WhatsApp Order Parser
          </h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              ...
              <textarea
                ...
                className="w-full h-48 bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-sm text-zinc-300 focus:border-amber-500 focus:outline-none resize-none transition-colors"
              />
              <button
              ...
              {parsedData ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-amber-500">₹{parsedData.total}</span>
                  </div>
                  <div className="pt-4 border-t border-zinc-800">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-zinc-500 text-sm">Generated Tracking ID</span>
                      <span className="font-mono text-amber-400 bg-amber-500/10 px-2 py-1 rounded text-xs">
                        {parsedData.id}
                      </span>
                    </div>
                  </div>
                  <button
                    ...
                    className="mt-6 w-full bg-amber-500 text-black font-bold py-3 rounded-xl hover:bg-amber-400 transition-colors flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(251,191,36,0.3)] hover:shadow-[0_0_30px_rgba(251,191,36,0.5)]"
                  >
                    ...
                    .map((order) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className="font-mono text-xs text-amber-500 bg-amber-500/10 px-2 py-1 rounded">
                            {order.id}
                          </span>
                          <button 
                        ...
                        <div className="flex gap-2 mt-4 pt-4 border-t border-zinc-900">
                          ...
                          {status !== "Delivered" && (
                            <button
                              onClick={() => updateOrderStatus(order.id, statuses[statuses.indexOf(status) + 1])}
                              className="flex-1 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 text-xs font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-1"
                            >
