            ...
            bookings.map((booking) => (
              <div key={booking.id} className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 bg-black/20 p-3 rounded-xl border border-zinc-800/50">
                  <div>
                    <div className="text-xs text-zinc-500 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3 text-[#D4AF37]"/> Date & Time</div>
                    <div className="text-sm text-white">{new Date(booking.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
                    <div className="text-sm text-[#D4AF37] font-semibold">{booking.timeSlot}</div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-500 mb-1 flex items-center gap-1"><MapPin className="w-3 h-3 text-[#D4AF37]"/> Event Info</div>
                ...
                {booking.requirements && (
                  <div>
                    <div className="text-xs text-zinc-500 mb-1 flex items-center gap-1"><AlignLeft className="w-3 h-3 text-[#D4AF37]"/> Requirements</div>
